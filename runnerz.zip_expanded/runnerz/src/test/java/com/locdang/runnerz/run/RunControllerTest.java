...
public class RunControllerTest {
	...
	void shouldFindAllRuns() throws Exception {
		...
		mvc.perform(get("/api/runs")).andExpect(status().isOk()).andExpect(jsonPath("$.size()", is(runs.size())));
	}
	...
	void shouldFindOneRun() throws Exception {
		...
		mvc.perform(get("/api/runs/1")).andExpect(status().isOk()).andExpect(jsonPath("$.id", is(run.id())))
				.andExpect(jsonPath("$.title", is(run.title()))).andExpect(jsonPath("$.miles", is(run.miles())))
	...
	void shouldCreateNewRun() throws Exception {
		var run = new Run(10, "test", LocalDateTime.now(), LocalDateTime.now(), 1, Location.INDOOR, 1);
		mvc.perform(
	...
	public void shouldDeleteRun() throws Exception {
		mvc.perform(delete("/api/runs/1")).andExpect(status().isNoContent());
	}
