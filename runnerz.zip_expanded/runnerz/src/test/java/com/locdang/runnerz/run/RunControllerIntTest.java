...
//
//		assertAll(() -> assertEquals(1, run.id()), () -> assertEquals("Noon Run", run.title()),
//				() -> assertEquals("2024-02-20T06:05", run.startedOn().toString()),
//				() -> assertEquals("2024-02-20T10:27", run.completedOn().toString()),
//				() -> assertEquals(24, run.miles()), () -> assertEquals(Location.INDOOR, run.location()));
//	}
//
//	@Test
...
//				Location.OUTDOOR, 1);
//
//		ResponseEntity<Void> newRun = restClient.post().uri("/api/runs").body(run).retrieve().toBodilessEntity();
//
...
//		Run run = restClient.get().uri("/api/runs/1").retrieve().body(Run.class);
//
//		ResponseEntity<Void> updatedRun = restClient.put().uri("/api/runs/1").body(run).retrieve().toBodilessEntity();
//
...
//		ResponseEntity<Void> run = restClient.delete().uri("/api/runs/1").retrieve().toBodilessEntity();
//
